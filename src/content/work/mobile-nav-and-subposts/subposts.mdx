...
One of the more intricate parts of this update was rethinking navigation. The original `getAdjacentWorks(){:ts}` function assumed simple previous/next relationships. With subposts, we now have three distinct navigation contexts:
...
- The breadcrumb component required careful thought to handle three distinct cases:
    ...
    ```astro title="src/pages/work/[...id].astro" startLineNumber={70}
    <Breadcrumbs
      items={[
        { href: '/work', label: 'Blog', icon: 'lucide:library-big' },
        ...(isCurrentSubpost && parentPost
          ? [
              {
                href: `/work/${parentPost.id}`,
                label: parentPost.data.title,
                icon: 'lucide:book-open',
              },
              {
                href: `/work/${currentPostId}`,
...
- The main blog listing (alongside other listings, e.g. filtering by tags, filtering by author) should exclude subposts to avoid cluttering the feed:
    ...
    export async function getAllWorks(): Promise<CollectionEntry<'blog'>[]> {
    const posts = await getCollection('blog')
    return posts
